...
4. **아웃룩 일정 30분 전 사전 승격 및 정시 프로그레스 정책**:
   - 아웃룩에 등록된 업무 일정은 시작 시각 30분 전(`st_dt - 30분`)부터 실시간 LIVE 관제(진행 중) 카드로 사전 승격되어 예정된 업무를 팀원들이 미리 인지할 수 있습니다.
   - 단, 프로그레스 바 진행률은 실제 업무 시작 시각(`st_dt`)에 맞추어 시작되며, 시작 전 대기 시간에는 프로그레스 바 0%(`width: 0%`) 및 `[대기 (0%)]`, `[시작예정 00:00]`, `⏱️ N분 후 시작`으로 명확하게 표출됩니다.
5. **아웃룩 종일(All-Day) 작업 일정의 09:00~18:00 표준 근무시간 보정 정책**:
   - 아웃룩에서 '종일'로 등록된 일반 작업 일정은 자정에 시작/종료되거나 조기 완료 처리되지 않도록, 시스템에서 당일 **09:00~18:00 (9.0h)** 정규 업무 시간으로 강제 자동 보정합니다.
   - 08:30부터 30분 전 사전 승격되어 `[종일예정 09:00]` 카드로 노출되며, 09:00~18:00 동안 `[☀️ 종일]` 및 `[종일 09:00~18:00]` 뱃지와 함께 실시간 프로그레스 바가 진행됩니다. 18:00 이후에는 완료 카드로 자동 전환됩니다.
