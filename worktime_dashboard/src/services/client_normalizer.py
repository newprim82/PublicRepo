...
COMPOUND_OUTLOOK_CLIENT_PATTERNS = [
    ...
    (re.compile(r"^(?:sh\s*)?(?:수협은행|수협중앙회|수협)", re.IGNORECASE), "수협은행"),
    (re.compile(r"^(?:nh\s*)?(?:농협은행|농협중앙회|농협)", re.IGNORECASE), "농협"),
    (re.compile(r"^(?:cu\s*)?(?:신협중앙회|신협)", re.IGNORECASE), "신협중앙회"),
    (re.compile(r"^금호\s*(?:석유화학|석화|폴리켐|피앤비|미쓰이)?", re.IGNORECASE), lambda m: normalize_client_name(m.group(0))),
