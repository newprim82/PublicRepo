...
class AIBriefingService:
    ...
    def _call_gemini_api(cls, facts: Dict[str, Any], api_key: str) -> Optional[Dict[str, str]]:
        ...
        prompt = f"""당신은 IT 인프라/엔지니어링 기술본부의 수석 운영 분석관입니다.
아래에 제공된 [정량 데이터 팩트 JSON]은 이번 기간의 실제 엔지니어 현장 지원 실적 통계입니다.

정형화된 고정 틀을 벗어나, 사람이 직접 작성한 것처럼 날카롭고 생생하며 누구나 쉽게 이해할 수 있는 [주간/월간 업무 실적 핵심 브리핑]을 작성해주세요.
...
"""
        model_candidates = ["gemini-3.5-flash-lite", "gemini-3.1-flash-lite", "gemini-3.6-flash"]
