...
def get_live_task_card_html(r, title_mappings, kst_now_naive, is_single_view: bool = False) -> str:
    ...
    if c_name in ["아웃룩 일정", "기타"] or "아웃룩" in str(c_name):
        ...
        if pc and pc not in ["아웃룩 일정", "기타"]:
            c_name = pc
            prefix = "[📅 아웃룩] " if "[아웃룩]" in str(t_desc) else ""
            t_desc = f"{prefix}{pt}"
    st_dt = r["start_time"]
