...
def main():
    ...
    if not df.empty and "week_label" in df.columns:
        ...
        if overwork_items:
            ...
            # 🟢 과중 근무자가 없는 경우: 차분한 정상 상태 배너 표출 (수직/수평 완벽 중앙 정렬 단일 Flexbox)
            with st.container(border=True):
                st.markdown("""
                <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; min-height: 36px; padding: 2px 2px; box-sizing: border-box;">
                    <div style="font-size: 14.5px; font-weight: 700; color: #E2E8F0; display: flex; align-items: center; gap: 10px;">
                        <span style="font-size: 16px; line-height: 1;">🟢</span>
                        <span class="normal-status-badge">[과중 근무 없음]</span>
                        <span style="color: #94A3B8; font-size: 13.5px;">현재 선택된 기간 내에 주 40시간 / 52시간을 초과한 과중 근무 팀원이 없습니다. (안정적인 근무 상태)</span>
                    </div>
                    <div>
