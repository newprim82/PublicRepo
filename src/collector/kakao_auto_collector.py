...
def find_kakao_chat_window(chat_title_keyword: str) -> Optional[int]:
    ...
    def enum_windows_callback(hwnd, _):
        ...
        if win32gui.IsWindowVisible(hwnd):
            ...
            if ("기술본부" in title_clean or "업무공유" in title_clean or target_clean in title_clean) and title != "카카오톡":
                candidates.append((hwnd, title, cls_name))
                found_hwnd = hwnd
                return False
        return True
    ...
    if found_hwnd:
        log_trace(f"[✓ 창 감지 성공] HWND={found_hwnd} | 제목='{win32gui.GetWindowText(found_hwnd)}' | 클래스='{win32gui.GetClassName(found_hwnd)}'")
    else:
...
def extract_text_from_kakao_window(hwnd: int) -> str:
    ...
    1순위: 대화 목록 영역(EVA_VH_ListControl_RPC) 전용 안전 복사
    2순위: UIA 심층 순회 (WalkTree)
    """
    ...
    log_trace(f"[자식 윈도우 확인] 총 {len(child_hwnds)}개 자식 창 | 대화목록 HWND={list_hwnd}")

    # [방법 1] 대화 목록 영역 포커스 및 클립보드 안전 복사
    try:
        # 기존 클립보드 데이터 백업
        old_clipboard = ""
        try:
        ...
        # 대화목록 창(EVA_VH_ListControl_RPC) 클릭으로 포커스
        target_focus = list_hwnd if list_hwnd else hwnd
        if target_focus:
            # 안전한 대화목록 영역 (중앙/상단 좌표) 클릭
        ...
        # UIAutomation 또는 Windows 키 이벤트를 통한 Ctrl+A -> Ctrl+C
        ...
        if copied_text and len(copied_text.strip()) > 10:
            ...
            log_trace(f"[✓ 클립보드 추출 성공] 총 {len(lines)}줄 ({len(copied_text)}자) 획득")
            return copied_text.strip()
        else:
            log_trace(f"[-] 클립보드 텍스트가 비어있음 (길이: {len(copied_text) if copied_text else 0})")
    except Exception as e:
        log_trace(f"[클립보드 추출 예외]: {e}")

    # [방법 2] UIA 전체 트리 심층 순회 (WalkTree Fallback)
    extracted_lines = []
    try:
        ctrl = auto.ControlFromHandle(list_hwnd if list_hwnd else hwnd)
        if ctrl:
            log_trace("[UIA] WalkTree 심층 순회 시작...")
            for c, depth in auto.WalkTree(ctrl, getChildren=auto.GetChildren):
                ...
                if name and len(name.strip()) > 0:
                    s = name.strip()
                    if not any(ign == s for ign in ["최소화", "최대화", "닫기", "전송", "메뉴", "검색", "이모티콘", "파일 보내기", "음성 대화", "페이스톡"]):
                        extracted_lines.append(s)
            log_trace(f"[UIA] WalkTree 완료. 수집 라인 수: {len(extracted_lines)}")
    except Exception as e:
        log_trace(f"[UIA WalkTree 예외]: {e}")

    if extracted_lines:
        return "\n".join(extracted_lines)

    log_trace("[-] 모든 추출 방식에서 텍스트 수집 실패")
    return ""
