...
3. **카카오톡 창 열어두기**:
   - PC 카카오톡에서 **`[기술본부] 업무공유방`** 채팅방 창을 열어 화면 한구석에 띄워둡니다.
   - 10분마다 자동으로 최신 대화를 긁어와 Supabase 클라우드에 실시간 저장합니다.

### 🔌 Windows 원격 데스크톱(mstsc) 종료 시 (화면 잠금 방지)
- **주의**: 원격 데스크톱 창 우측 상단의 `X` 버튼을 누르면 Windows 화면이 잠겨 카톡 GUI 복사가 중단됩니다.
- **해결책**: 원격을 종료할 때는 폴더 내 **`disconnect_keep_gui.bat`**을 더블 클릭하여 종료합니다. (로컬 콘솔 모드로 전환되어 24시간 정상 수집 유지)

### 💻 PC A & PC C (다른 팀원 PC / 모바일 접속)
* 웹 브라우저(Chrome, Edge) 주소창에 접속:  
  `http://[PC_B의_IP주소]:8501` (예: `http://172.16.6.126:8501`)

---

## 🛠️ 7. 유지보수 및 배치 자동화 도구

| 도구 / 스크립트 | 설명 |
| :--- | :--- |
| **`update_and_run.bat`** | 기존 대시보드 프로세스를 안전하게 종료하고, 최신 코드를 git pull 받아 캐시를 정리한 뒤 새로 실행합니다. |
| **`disconnect_keep_gui.bat`** | **[필수]** 원격 데스크톱(mstsc) 접속 후 화면 잠금(Lock) 없이 로컬 콘솔로 전환하며 창을 닫습니다. |
| **`src/scripts/send_weekly_report.py`** | Windows 작업 스케줄러와 연동하여 매주/매월 정기 업무 실적 Summary를 자동 발송하는 CLI 배치 스크립트. |
