...
3. **48시간 자동 완료 (Auto-Complete) 정책**:  
   - 완료보고를 누락한 채 48시간이 경과한 PENDING 건은 **'예정 시간'을 '실제 시간'으로 인정하여 `COMPLETED`로 자동 전환**.
   - 비고란에 `[자동완료] 48시간 경과로 시작보고 기준 완료 처리` 명시.
4. **다일(Multi-day) 작업 일자별 자동 분할**:
   - `3days` 작업은 시작일부터 주말을 고려하여 **하루 9.0시간씩 3개의 개별 일자 레코드**로 분할 생성하여 일별/주별 공수 왜곡을 방지.
...
* **표시 및 경고 기준**:
  - **주 40h 초과 (주의)**: `⚠️ 홍길동(8월 2주차: 41.0h)` ➔ 오렌지색 배너 & 칩 표시.
  - **주 52h 초과 (위험)**: `🚨 김철수(8월 3주차: 54.5h)` ➔ 붉은색 네온 플래시 깜빡임 배너 표시.
